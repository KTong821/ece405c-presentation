...
\begin{frame}
    ...
    \begin{itemize}
        \item Brute Force
        \item Boyer-Moore
        \item Suffix Tree/Array
        \item Karp-Rabin
    \end{itemize}
    TODO - add details (runtime), to above. more for karp-rabin, which polynomial is similar to
    \bigskip
    \begin{itemize}
        \item \alert{Polynomial Matching}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    Notice that $\|P\|^2 = 11^2 + 5^2 + 11^2 + (-3)^2 = 276$ \\
    \bigskip
    No more exact matches, but highest values correspond to likeliest index.
\end{frame}
...
\begin{frame}
    ...
    Talk about naive $O(n^2)$, Karatsuba, and FFT
